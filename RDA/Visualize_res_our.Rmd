...
params: 
  ...
  data_folder: "../../Results/Rdata/RDA/CDF_freqtrun5/"
---
...
# pdf_array_list = list(center_pdf_array_list[[1]][order_clus_1,order_clus_1, ,drop=FALSE],center_pdf_array_list[[2]][order_clus_2,order_clus_2, ,drop=FALSE])
# pdf_true_array = (pdf_array_list[[1]]+pdf_array_list[[2]])/2
# g3 = plot_pdf_array_v2(pdf_array_list = pdf_array_list, 
#                         pdf_true_array = pdf_true_array,
#                         clus_size_vec = clus_size_list[[1]][order_clus_1]+clus_size_list[[2]][order_clus_2],
#                         t_vec = t_vec, x_lim = c(), y_lim = c(0,max(unlist(center_pdf_array_list[[2]]))))
# g3
...
```{r}
ind_vec = which(unlist(v_vec_list)!=0 & activeTime_vec==1)
ind = ind_vec[1]

for (i in 1:min(length(ind_vec),3)) {
  ind = ind_vec[i]
  data_trace = tibble(dFF=dFF_mat[ind,],time=(1:ncol(dFF_mat)*0.25)/60)
  g = ggplot(data_trace, aes(x=time, y=dFF)) +
    geom_line(alpha=0.3) +
    # xlim(unlist(v_vec_list)[ind]+c(-1,1)) +
    geom_vline(xintercept = unlist(v_vec_list)[ind])
  print(g)
}
