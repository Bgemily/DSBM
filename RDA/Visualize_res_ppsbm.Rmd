...
```{r}
### Read in data analysis results
data_folder = paste0("../../Results/Rdata/RDA/ppsbm/", params$subj_name, "/") 
...
g1 = plot_pdf_array_v2(pdf_array_list = center_pdf_array_list[[1]], 
                        pdf_true_array = center_pdf_array_list[[1]],
                        clus_size_vec = clus_size_list[[1]],
                        t_vec = t_vec, y_lim = c(0,max(unlist(center_pdf_array_list))))

g2 = plot_pdf_array_v2(pdf_array_list = center_pdf_array_list[[2]], 
                        pdf_true_array = center_pdf_array_list[[2]],
                        clus_size_vec = clus_size_list[[2]],
                        t_vec = t_vec, y_lim = c(0,max(unlist(center_pdf_array_list))))
...
### Fix bug in ppsbm and order clusters by connecting probability
```{r}
### Fix bugs in ppsbm
for(ind_subj in 1:length(center_pdf_array_list)){
  center_pdf_array_list[[ind_subj]][center_pdf_array_list[[ind_subj]]==1] = 0
}
...
g1 = plot_pdf_array_v2(pdf_array_list = center_pdf_array_list[[1]][order_clus_1,order_clus_1,], 
                        pdf_true_array = center_pdf_array_list[[1]][order_clus_1,order_clus_1,],
                        clus_size_vec = clus_size_list[[1]][order_clus_1],
                        t_vec = t_vec, y_lim = c(0,max(unlist(center_pdf_array_list))))

g2 = plot_pdf_array_v2(pdf_array_list = center_pdf_array_list[[2]][order_clus_2,order_clus_2,], 
                        pdf_true_array = center_pdf_array_list[[2]][order_clus_2,order_clus_2,],
...
ggplot(data = data, mapping = aes(x=LR, y=AP, color=membership)) +
  geom_point() + 
  scale_color_manual(values = c(`1`='red',`2`='green',`3`='blue',`0`='gray'))
