...
subj_name_list = c(subj_name_list)


edge_time_mat_list = locs_mat_list = 
  mnx_vec_list = islet_vec_list = 
...
for(m in 1:length(path_vec)){
  ...
  if(m==4|m==5){
    islet_all = as.matrix(read.csv(paste(data_folder,path,'/islet.csv',sep='')))
    islet_all = islet_all[,-1]
    islet_vec = islet_all[avai.inds]
  }
  ...
  if(m==4|m==5){
    islet_vec_L = islet_vec[inds_L]
    islet_vec_R = islet_vec[inds_R]
  }
  ...
  if (m==4|m==5) {
    islet_vec_list[c(2*m-1,2*m)] = list(islet_vec_L,islet_vec_R)
  }
...
max_time = max(sapply(edge_time_mat_list[1:2], function(edge_time_mat)max(edge_time_mat[which(edge_time_mat<Inf)])))
t_vec = seq(0, max_time+10, length.out = 1000)


for (N_clus in 3:3) {
  res = get_init_v4(edge_time_mat_list = edge_time_mat_list[1], N_clus = N_clus, 
                    t_vec = t_vec)
  ...
  # dir.create(paste0('../Results/Rdata/real_data_results/', now), recursive = TRUE, showWarnings = FALSE)
  
  # avai_subj = c(1:4, 7:10, 13:14)
  avai_subj = c(2)
  res = do_cluster_v14(edge_time_mat_list = edge_time_mat_list[avai_subj], N_clus = N_clus, 
                         clusters_list_init = clusters_list_init[avai_subj], 
                         n0_vec_list_init = n0_vec_list_init[avai_subj], 
                         n0_mat_list_init = n0_mat_list_init[avai_subj], 
  ...
  center_pdf_array = get_center_pdf_array_v2(edge_time_mat_list = edge_time_mat_list[avai_subj], 
                                                 ...
                                                 t_vec = t_vec, bw=bw)
  center_pdf_array_list = list(center_pdf_array)
  ...
  save(list=c('subj_name_list', 'avai_subj',
       ...
       file=paste0('../Results/Rdata/real_data_results/',
                   'Single_subj', '_', avai_subj, '_', 'N_clus', N_clus, '.Rdata'))
...
# for (N_subj in c(2,4,8,12,16)) {
#   
#   now = format(Sys.time(), "%Y%m%d_%H%M%S")
#   dir.create(paste0('../Results/Rdata/real_data_results/', now), recursive = TRUE, showWarnings = FALSE)
#   
#   res = do_cluster_v11.1(edge_time_mat_list = edge_time_mat_list[1:N_subj], N_clus = N_clus, 
#                        clusters_list_init = clusters_list_init[1:N_subj], 
#                        n0_vec_list_init = n0_vec_list_init[1:N_subj], 
#                        n0_mat_list_init = n0_mat_list_init[1:N_subj], 
#                        t_vec = t_vec, bw = bw)
#   
#   res$clusters_list -> clusters_list
#   res$clusters_history -> clusters_history
...
#               'v_vec_list', 'v_vec_list_init',
#               'center_pdf_array', 'center_pdf_array_list'),
#        file=paste0('../Results/Rdata/real_data_results/', now, '/',
#                    'All_subj', '_','N_subj', N_subj, '.Rdata'))
#   
#   
# }
