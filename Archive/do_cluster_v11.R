...
### Based on v10
### Stage I: cluster_kmeans_v4 --- force the number of clusters using k-medoid. 
### Stage II: cluster_kmeans_v6 --- force n0 to be less than earliest edge time, and consider conn_prob.

do_cluster_v11 = function(edge_time_mat_list, N_clus, 
                         clusters_list_init, n0_vec_list_init, n0_mat_list_init,
                         total_time = 200, t_vec=seq(0,total_time,length.out=1000),
                         MaxIter=10, conv_thres=5e-3, bw=5, gamma=10, ...)
{
...
# Update clusters and connecting patterns separately for each subject ---------------------
  ...
  for (m in 1:N_subj) {
    ...
    while (!stopping & n_iter<=MaxIter){
      ...
      res = cluster_kmeans_v4(edge_time_mat_list=edge_time_mat_list[m],
...
# Combine all subjects and update params ----------------------------
  ...
  if (N_subj>1) {
    ...
    while (!stopping & n_iter<=MaxIter){
      ...
      ### Update clusters, time shifts and connecting patterns
      res = cluster_kmeans_v6(edge_time_mat_list=edge_time_mat_list, 
                              clusters_list=clusters_list_current, 
                              n0_vec_list=n0_vec_list_current, n0_mat_list=n0_mat_list_current, 
                              center_cdf_array = center_cdf_array_current,
                              t_vec=t_vec, order_list=NULL, gamma=gamma, ...)
