...
### Based on v9
### Use k-medoid for individual subjects to guarantee there are N_clus clusters.

do_cluster_v10 = function(edge_time_mat_list, N_clus, 
...
# Combine all subjects and update params ----------------------------
  ...
  if (N_subj>1) {
    ...
    while (!stopping & n_iter<=MaxIter){
      ...
      ### Update clusters, time shifts and connecting patterns
      res = cluster_kmeans_v5(edge_time_mat_list=edge_time_mat_list, 
                              clusters_list=clusters_list_current, 
    ...
    if (n_iter>MaxIter) {
      message("[do_cluster_v10]: Reached maximum iteration number.")
    }
