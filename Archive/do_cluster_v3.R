...
### main algorithm
### At the beginning, not forcing the true order. After convergence/stuck, force the true order. 
### Observe the loss function values after each iteration.
do_cluster_v3 = function(edge_time_mat_list, N_clus, 
                         total_time = 200, t_vec=seq(0,total_time,length.out=1000),
                         MaxIter=10, conv_thres=5e-3, order_list=NULL, ...)
{
...
# Initialize clusters and time shifts -------------------------------------
  ...
  res = get_init_v2(edge_time_mat_list=edge_time_mat_list, N_clus=N_clus, 
...
# Update clusters and connecting patterns alternatively ---------------------
  ...
  print("#################")
  print(paste(n_iter-1, "iterations done...","Forcing the true order..."))
  print("#################")

# Force the true order and update params again ----------------------------

  n_iter = 1
  stopping = FALSE
  
  ### Force the order to be the true order
  for (m in 1:N_subj) {
    n0_vec_list_current[[m]][order_list[[m]]] = sort(n0_vec_list_current[[m]])
  }
  n0_mat_list_current = n0_vec2mat(n0_vec = n0_vec_list_current)
  ### Evaluate loss function
  loss = eval_loss_v2(edge_time_mat_list = edge_time_mat_list, 
