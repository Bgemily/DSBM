...
### ERROR: this function should NOT estimated clusters based on all subjects.
get_init_v2 = function(edge_time_mat_list, N_clus, t_vec=seq(0, 50, 0.05), order_list=NULL){
  ...
  for (m in 1:N_subj) {
    ...
    if (!is.null(order_list)) {
      n0_vec[order_list[[m]]] = sort(n0_vec)
    }
