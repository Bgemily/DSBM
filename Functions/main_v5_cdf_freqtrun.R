...
main_v5_cdf_freqtrun = function(### Parameters for generative model
  ...
  if (N_clus_est == N_clus) {
    ...
    if (save_est_history==TRUE) {
      ...
      for (ind_iter in 1:length(clusters_history)) {
        ...
        ### Calculate F_mse
        ### Get estimated pdf using frequency truncation
        clusters_list_tmp = list(clusters_history[[ind_iter]])
        n0_mat_list_tmp = list(n0_vec2mat(v_vec_history[[ind_iter]] / (t_vec[2]-t_vec[1])))
        center_fft_array_tmp = get_center_fft_array(edge_time_mat_list = edge_time_mat_list, 
                                                clusters_list = clusters_list_tmp, 
                                                freq_trun = freq_trun_calc_mse, 
                                                n0_mat_list = n0_mat_list_tmp, t_vec = t_vec)
        center_pdf_array_tmp = array(dim=c(N_clus, N_clus, length(t_vec)))
        for (q in 1:N_clus) {
          for (k in 1:N_clus) {
            fft_truncated = center_fft_array_tmp[q,k,]
            func = Re(fft(c(tail(fft_truncated, freq_trun+1), 
                            rep(0, length(t_vec)-2*freq_trun-1),
                            head(fft_truncated, freq_trun)), inverse = TRUE))
            center_pdf_array_tmp[q,k,] = func
          }
        }
