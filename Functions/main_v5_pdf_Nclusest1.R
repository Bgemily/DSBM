...
main_v5_pdf_Nclusest1 = function(### Parameters for generative model
  ...
  ...)
{
  ...
  if (N_clus_est == 1) {
    ...
    center_pdf_array_est_permn = center_pdf_array_est
    ...
    for (q in 1:N_clus) {
      for (k in 1:N_clus) {
        dist_mat[q,k] = sqrt(sum( (center_pdf_array_est_permn[1,1,] - pdf_true_array[q,k,])^2 * 
                                    (t_vec[2]-t_vec[1]) ))
      }
    }
